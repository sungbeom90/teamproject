...
public class OfferController {
	...
	public String barcelona_offer2(Model model) {
		...
		board.setInclude("<li>전문 선장이 운항하는 2시간의 항해 여행</li>"
				+ "<li>간식,음료수</li>"
				+ "<li>스패니쉬 기타연주</li>");
		board.setUninclude("<li>호텔 픽업 및 샌딩</li>"
				+ "<li>기타 음식 및 음료</li>");
		board.setCourse("<p>바르셀로나 포트벨 일몰항해투어 : 붉게 물든 바다 위에서 바르셀로나의 석양을 즐겨 보세요!</p>");
		board.setAddinfo("<p>- 여행확인증이 아닌, 영어로 된 바우처를 지참하셔야 합니다.</p>" + 
				"<p>- 바우처 형태 : 모바일 제시 / 출력본 제시</p>" + 
	...
	public String seville_offer1(Model model) {
		...
		board.setSubcontent("<p>스페인 광장부터 세비야 대성당 내부까지!" + 
				"<p>세비야의 주요 명소를 명쾌한 해설과 함께 둘러보는 시간!" + 
				"<p>저희와 함께 세비야의 매력에 푹 빠져보세요!" +  
				"<p>탄탄한 구성! 단 한 번의 투어로 세비야 주요 명소를 알차게 만나보세요." +
		...
		board.setUninclude("대성당 입장료" + 
				"<li>성인 : 10유로" + 
				"<li>학생 (국제학생증 소지, 만 25세 이하) : 5유로" + 
				"<li>경로 (여권 소지, 만 65세 이상) : 5유로" + 
				"<li>어린이 (여권 소지, 만 14세 이하) : 무료" + 
				"<li>개별 구매한 입장권 사용 가능");
		...
		board.setAddinfo("<p>- 투어 공지 및 세부 안내 사항은 마이리얼트립 메시지를 통해 전달됩니다. 메시지를 꼭 확인해 주세요." + 
				"<p>- 수신기 사용을 위한 이어폰은 꼭 지참해 주세요." + 
				"<p>- 약속시간을 지켜 주세요. 늦으시면 기다려 드릴 수 없습니다." + 
	...
	public String seville_offer2(Model model) {
		...
		board.setSubcontent("<p>★‘론다는 낮에도 좋은데 밤이 더 좋아’" + 
				"<p>모든 여행객이 입을 모아 하는 이야기지요." + 
				"<p>이 작은 도시에 매년 많은 여행객이 찾는 이유도 론다 특유의 낭만 때문이 아닐까 싶어요." + 
				"<p>하지만 론다에서 세비야 가는 시외버스 막차는 오후 6시 (좌절)" + 
				"<p>론다 야경을 보기에는 불가한 일정이지요. (노을 지는 모습은 뽀나스~!)" + 
				"<p>그렇다면 정답은? 세비야에서 출발하는 론다 별밤 투어!" + 
				"<p>혼자 오시는 손님도 환영합니다." + 
				"<p>혼자 오시는 분들이 많은 투어이며, 함께 여행하며 친해질 수 있어요!");
		board.setInclude("<li>전문 가이드비"
				+ "<li>전용 차량비"
				+ "<li>로컬가이드비"
				+ "<li>무선 수신기 대여비 (개인 이어폰 필수 지참)");
		board.setUninclude("<li>개인 이어폰 지참"
				+ "<li>저녁 식사비"
				+ "<li>개인음료 및 간식비");
	...
	public String busan_offer1(Model model) {
		...
		board.setSubcontent("<p>출발하기전 사진 찍는 방법을 먼저 안내드릴 거예요." + 
				"<p>날씨가 좋은 날에는 배수지 전망대에서 부산의 모든 야경을 보여드릴거에요." + 
				"<p>또 별이 보이는 날에는 별자리를 알려드릴 거구요." + 
				"<p>휴대폰으로 이쁜 야경사진을 찍는 방법도 안내드릴 거예요." + 
				...
				"<p>2~3시간 정도 소요되며, 촬영한 사진은 메일 또는 메신저로 전송해 드립니다!" +  
				"<p>투어 종료 후 숙소까지 모셔다 드릴 거예요~" + 
				"<p>예쁜 사진을 찍기 위해서는 무조건 밝은 계열의 옷을 입고 오셔야 합니다!!");
		board.setInclude("<li>사진촬영" + 
				"<li>투어 가이드" + 
				"<li>숙소 샌딩");
		board.setUninclude("<li>개인 경비");
		board.setCourse("<p>숨겨진 부산의 야경명소를 소개합니다!" + 
				"<p>부산에서의 야경이 가장 아름다운 장소로 갈 거예요" + 
				"<p>부산사람도 잘 모르는 야경 장소인 배수지 전망대로 출발합니다." + 
				"<p>전망대에서는 부산항이 잘보이는 곳으로 갑니다." + 
				"<p>전망대에는 사람이 많이 없어 사진 찍기가 엄청 편하답니다~");
		board.setAddinfo("<p>최대 승차인원은 4인입니다." + 
				"<p>필요 시 여행자보험에 가입 후 참여하실 것을 권장합니다." + 
				"<p>1인만 예약해도 투어 진행이 가능합니다");
	...
	public String busan_offer2(Model model) {
		...
		board.setSubcontent("<p>별도의 사전 예약 없이 이용하시는 경우 만석으로 이용이 어려울 수 있으니 업체에 사전 유선 예약 진행 필요합니다."
				+ "<p>명품브랜드 Lagoon 카타마란 요트를 타고 해운대 광안리 요트 투어를 즐기자!"
				+ "<p>무료 음료, 간식을 제공합니다.");
		...
		board.setContacttime("<p>일정은 상담을 통해 자유롭게 변경 및 조율 가능합니다." + 
				"<p>주간투어 : [13:00] [14:00] [15:00] [16:00] [17:00]\"" + 
	...
	public String okinawa_offer1(Model model) {
		...
		board.setUninclude("<li>교통비" + 
				"<li>시설 내의 유료 입장 시설, 유료 놀이기구 및 음식대금" + 
				"<li>기타 개인 지출 비용");
		board.setCourse("<ol><li>오키나와 추라우미 수족관 <li>다이세키린잔 <li> 고우리 오션타워<li> 류구조<li> OKINAWA 후루츠랜드<li> 나고 파인애플 파크<li> 네오파크 오키나와<li> 류큐무라<li> 무라사쿠무라 체험<li> 비오스의 언덕<li> 오키나와 월드 교쿠센도<li> 오키나와 평화 기념당");
		board.setAddinfo("<p>이용일 1일 전 취소 및 환불은 무료로 가능합니다." + 
				"<p>- 이용일 당일에 취소 및 환불은 불가합니다." + 
	...
	public String okinawa_offer2(Model model) {
		...
		board.setCourse("<h5>오키나와 게라마 제도 1일 투어</h5>" + 
				"<p>07:30-08:00 고객께서 지정하신 나하 시내 호텔로 픽업(예약 시에 선택한 호텔)" + 
		...
		board.setAddinfo("<p>- 수영복과 비치샌들을 지참해 주십시오. 보트내의 탈의실에서 수영복으로 갈아 입을 수 있습니다." + 
				"<p>- 또한, 수영복과 비치샌들 등의 판매와 대여는 없습니다."
				+ "<p>- 베지테리언식을 희망하시는 고객은 예약시에 신청해 주세요."
